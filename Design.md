========================================
Timecard工时管理系统 - 详细设计文档
========================================

本文档详细记录Timecard工时管理系统中每个TSX组件的设计风格、设计要素和实现细节。
...
【设计定位】
系统顶层容器，负责整体布局和页面路由管理

【设计风格】
- 极简化布局：采用固定头部（sticky header）+ 容器化内容区域的经典布局模式
- 扁平化导航：使用Tabs组件实现单页面多模块切换，避免页面跳转
- 轻量级状态：仅维护activeTab状态，采用受控组件模式

【设计要素】

A. 布局结构
- 外层容器：min-h-screen bg-slate-50 确保全屏覆盖和浅灰背景
- 头部区域：bg-white border-b border-slate-200 sticky top-0 z-50
  * 标题：Timecard Management System
  * 副标题：Track hours, manage projects, and generate reports
  * 内边距：container mx-auto px-4 py-4
- 主内容区：container mx-auto px-4 py-6

B. 导航系统
- TabsList配置：grid w-full max-w-2xl mx-auto grid-cols-4
- 四个Tab标签页：
  1. Timecard - Calendar图标
  2. Holidays - FileText图标
  3. Projects - Briefcase图标
  4. Reports - BarChart3图标
- 响应式文本：hidden sm:inline 保证移动端仅显示图标

C. 组件集成
- 状态管理：useState维护activeTab
- 路由映射：TabsContent与子组件一一对应
- Toast通知：全局Toaster组件用于消息提示

【技术细节】
- 图标库：lucide-react
- UI组件：shadcn/ui Tabs系统
- 状态初始值：'timecard'（默认打开工时卡页面）
