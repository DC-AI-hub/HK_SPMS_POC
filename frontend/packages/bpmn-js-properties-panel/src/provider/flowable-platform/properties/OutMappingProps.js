...
/**
 * Cf. https://docs.flowable.org/manual/latest/reference/bpmn20/custom-extensions/extension-elements/#out
 */
...
function addFactory({ bpmnFactory, commandStack, element }) {
  return function(event) {
    ...
    const newMapping = createElement('flowable:Out', {
...
function getOutMappings(element) {
  const businessObject = getBusinessObject(element);
  const mappings = getExtensionElementsList(businessObject, 'flowable:Out');
  ...
  // mapping.variables => flowable-platform/OutMappingPropagationProps
