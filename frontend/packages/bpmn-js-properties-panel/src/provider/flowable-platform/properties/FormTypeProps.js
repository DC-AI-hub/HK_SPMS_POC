...
const FORM_KEY_PROPS = {
  'flowable:formRef': undefined,
  'flowable:formRefBinding': undefined,
  'flowable:formRefVersion': undefined
};

const FORM_REF_PROPS = {
  'flowable:formKey': undefined
};
...
function FormType(props) {
  ...
  const getValue = () => {
    if (isDefined(businessObject.get('flowable:formKey'))) {
      return 'formKey';
    } else if (isDefined(businessObject.get('flowable:formRef'))) {
      return 'formRef';
  ...
  const setValue = (value) => {
    ...
    if (value === 'formData') {
      ...
      // (2) create flowable:FormData
      const parent = extensionElements;

      const formData = createElement('flowable:FormData', {
        fields: []
      }, parent, bpmnFactory);
  ...
  const getOptions = () => {
    return [
      ...
      { value: 'formRef', label: translate('Flowable Forms') },
...
function getFormData(element) {
  ...
  return getExtensionElementsList(bo, 'flowable:FormData')[0];
}
