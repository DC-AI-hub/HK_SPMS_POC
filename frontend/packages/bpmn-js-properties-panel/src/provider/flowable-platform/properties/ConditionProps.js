 ...
 * Cf. https://docs.flowable.org/manual/latest/reference/bpmn20/gateways/sequence-flow/
 */
...
function ScriptType(props) {
  ...
  const setValue = (value) => {
    ...
    const updatedProperties = {
      ...
      'flowable:resource': value === 'resource' ? '' : undefined
    };
...
function Resource(props) {
  ...
  const getValue = () => {
    return getConditionExpression(element).get('flowable:resource');
  };
...
function VariableEvents(props) {
  ...
  const tooltip = (
    <div>
      ...
      <a href="https://docs.flowable.org/manual/latest/reference/bpmn20/custom-extensions/extension-attributes/#variableevents" target="_blank" rel="noopener noreferrer">{ translate('Documentation: Variable events') }</a>
    </div>
  );
...
function getScriptType(element) {
  ...
  const resource = conditionExpression.get('flowable:resource');
