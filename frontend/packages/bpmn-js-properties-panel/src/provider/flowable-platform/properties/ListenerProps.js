...
/**
 * Cf. https://docs.flowable.org/manual/latest/user-guide/process-engine/delegation-code/#execution-listener
 */
export function ExecutionListenerProps({ element, injector }) {
  ...
  const listeners = getExtensionElementsList(businessObject, 'flowable:ExecutionListener');
...
export function TaskListenerProps({ element, injector }) {
  ...
  const businessObject = getListenersContainer(element);
  const listeners = getExtensionElementsList(businessObject, 'flowable:TaskListener');
...
function EventType({ id, element, listener }) {
  ...
  function getOptions() {
    ...
    if (is(listener, 'flowable:TaskListener')) {
      return [
...
function ListenerId({ id, element, listener }) {
  ...
  let options = {
    ...
    getValue: () => {
      return listener.get('flowable:id');
    }
  };
...
function Fields(props) {
  ...
  function addField() {
    const field = createElement('flowable:Field', {}, listener, bpmnFactory);
...
function addTaskListenerFactory(props) {
  return addListenerFactory({ ...props, listenerGroup:'flowable:TaskListener' });
}

function addExecutionListenerFactory(props) {
  return addListenerFactory({ ...props, listenerGroup:'flowable:ExecutionListener' });
}
...
function getDefaultImplementationProperties(type, bpmnFactory) {
  ...
  case 'script':
    return { ...DEFAULT_PROPS, 'script': bpmnFactory.create('flowable:Script') };
  }
}
