...
export function JobExecutionProps(props) {
  ...
  // (1) add retryTimeCycle field for flowable:asyncCapable enabled Elements
  // or TimerEvents
  if ((is(element, 'flowable:AsyncCapable') && isAsync(businessObject)) ||
  ...
  // (2) add jobPriority field for flowable:jobPriorized with async enabled
  //  or Processes
  //  or Processes referred to by participants
  //  or TimerEvents
  if ((is(element, 'flowable:JobPriorized') && isAsync(businessObject)) ||
...
function JobPriority(props) {
  ...
  const getValue = () => {
    return businessObject.get('flowable:jobPriority');
  };
...
function RetryTimeCycle(props) {
  ...
  const getValue = () => {
    const failedJobRetryTimeCycle = getExtensionElementsList(businessObject, 'flowable:FailedJobRetryTimeCycle')[0];
  ...
  const setValue = (value) => {
    ...
    let failedJobRetryTimeCycle = getExtensionElementsList(businessObject, 'flowable:FailedJobRetryTimeCycle')[0];
