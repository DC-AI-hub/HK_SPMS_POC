 ...
 * Cf. https://docs.flowable.org/manual/7.15/reference/bpmn20/subprocesses/call-activity/#calledelement-tenant-id
 */
export function CalledBpmnProps(props) {
  ...
  const binding = getBusinessObject(element).get('flowable:calledElementBinding');
  if (binding === 'version') {
    entries.splice(-1, 0,
      {
...
function CalledElementBinding(props) {
  ...
  const getValue = () => {
    return getBusinessObject(element).get('flowable:calledElementBinding') || 'latest';
  };
...
function CalledElementVersion(props) {
  ...
  const getValue = () => {
    return getBusinessObject(element).get('flowable:calledElementVersion');
  };
...
function CalledElementVersionTag(props) {
  ...
  const getValue = () => {
    return getBusinessObject(element).get('flowable:calledElementVersionTag');
  };
...
function CalledElementTenantId(props) {
  ...
  const getValue = () => {
    return getBusinessObject(element).get('flowable:calledElementTenantId');
  };
