...
function ExternalTaskPriority(props) {
  ...
  const getValue = () => {
    return businessObject.get('flowable:taskPriority');
  };
...
function isExternalTaskLike(element) {
  const bo = getServiceTaskLikeBusinessObject(element),
        type = bo && bo.get('flowable:type');

  return is(bo, 'flowable:ServiceTaskLike') && type && type === 'external';
}
