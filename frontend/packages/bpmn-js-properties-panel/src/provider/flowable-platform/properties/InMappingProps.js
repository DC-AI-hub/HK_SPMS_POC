...
/**
 * Cf. https://docs.flowable.org/manual/latest/reference/bpmn20/custom-extensions/extension-elements/#in
 */
...
function addFactory({ bpmnFactory, commandStack, element }) {
  return function(event) {
    ...
    const newMapping = createElement('flowable:In', {
...
function getInMappings(element) {
  ...
  const mappings = getExtensionElementsList(signalEventDefinition || businessObject, 'flowable:In');

  // only retrieve relevant mappings here, others are handled in other groups
  // mapping.businessKey => flowable-platform/CallAvtivityProps
  // mapping.variables => flowable-platform/InMappingPropagationProps
