...
/**
 * Get a flowable:inputOutput from the business object
 *
...
export function getInputOutput(element) {
  if (is(element, 'flowable:Connector')) {
    return element.get('inputOutput');
  }

  const businessObject = getBusinessObject(element);

  return (getElements(businessObject, 'flowable:InputOutput') || [])[0];
}
...
export function CreateParameterCmd(element, type, parent, bpmnFactory) {
  const isInput = type === 'flowable:InputParameter';
...
export function AddParameterCmd(element, type, bpmnFactory) {
  ...
  if (!inputOutput) {
    ...
    inputOutput = createElement('flowable:InputOutput', {
      inputParameters: [],
      outputParameters: []
    }, parent, bpmnFactory);
